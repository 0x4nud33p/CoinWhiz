...
const Dropdown = ({ onSelect }) => {
  ...
  return (
    <div className="relative inline-block ml-4 text-left">
      ...
      {isOpen && (
        <ul className="absolute z-10 w-30 mt-1 text-[#68007a] border border-[#68007a] rounded-md shadow-lg"> {/* Set width to match search bar */}
          {options.map((option) => (
            <li
              key={option}
              className={`px-4 py-2 cursor-pointer hover:bg-[#b978c6] ${option === selected ? 'bg-[#b978c6]' : ''}`}
